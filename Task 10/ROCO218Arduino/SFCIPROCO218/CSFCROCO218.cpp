...
CSFCROCO218::CSFCROCO218(double A[][3], double B[],  double C[], double K[], double L[], double setPointAngle) {
...
double CSFCROCO218::ComputeSFC(double y, unsigned long theTime)
{
  ...
  // calculate observer correction term
    ...
    errorCorrection = (y -setPointAngle) - C[0]*xhat[0] - C[1]*xhat[1];
  // update the state estimates for theta and thetaHat
    xhat[0] += h*(A[0][0] * xhat[0] + A[0][1] * xhat[1] + B[0] * u + L[0] * errorCorrection);
    xhat[1] += h*(A[1][0] * xhat[0] + A[1][1] * xhat[1] + B[1] * u + L[1] * errorCorrection);
